    ...
    if(wPath.length() == 1) { // root folder of plugin
        ...
        if(status != 0) {
            ...
            gLogProc(gPluginNumber, MSGTYPE_IMPORTANTERROR, 
                wcharstring((WCHAR*)u"Command (rclone listremotes) exited with status ").append(int_to_wcharstring(status)).data()
            );
            return (HANDLE)-1;
        }
        ...
        {
            gLogProc(gPluginNumber, MSGTYPE_IMPORTANTERROR, UTF8toUTF16(e.what()).data());
        }
    }
